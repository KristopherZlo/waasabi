...
function Editor({post, kind, categories, mediaTypes, licenses, journal}: EditorProps) {
    ...
    return <><Head title={kind === 'update' ? t.write_update : post ? t.edit : kind === 'project' ? t.new_project : t.new_work}/>
        ...
        <Form action={destination} method={journal?.update ? 'put' : 'post'} onSuccess={clear} className="compose-form" preventEnterSubmit>
            ...
            {!journal && <section className="compose-details-page" hidden={stage !== 'details'}><div className="compose-details-head"><h1>{t.details}</h1><p>{t.details_hint}</p></div><div className="compose-settings"><SelectMenu name="feedback_mode" label={t.response} value={fields.feedback_mode} onChange={value => set('feedback_mode', String(value))} options={[{value: 'sharing', label: t.sharing}, {value: 'feedback', label: t.feedback}, {value: 'help', label: t.help_mode}]}/>
                    ...
                    <details open><summary><span>{t.category} & {t.tags.toLowerCase()}</span><ChevronDown size={17}/></summary><SelectMenu name="category" label={t.category} value={fields.category} onChange={value => set('category', String(value))} options={Object.entries(categories).map(([value, label]) => ({value, label}))}/><SelectMenu name="media_type" label={t.media_type} value={fields.media_type} onChange={value => set('media_type', String(value))} options={Object.entries(mediaTypes).map(([value, label]) => ({value, label}))}/><SelectMenu name="license" label={t.license} value={fields.license} onChange={value => set('license', String(value))} options={Object.entries(licenses).map(([value, label]) => ({value, label}))}/>{field('tags', t.tags)}</details>
                    ...
                    {kind === 'project' && <SelectMenu name="status" label={t.status} value={fields.status} onChange={value => set('status', String(value))} options={['in_progress', 'done', 'paused'].map(value => ({value, label: t[value]}))}/>}
                    <SelectMenu name="visibility" label={t.visibility} value={fields.visibility} onChange={value => set('visibility', String(value))} options={[{value: 'public', label: t.public}, {value: 'unlisted', label: t.unlisted}]}/>
