...
import { Check, ExternalLink, Pencil, Plus, Search, Send, Settings, Trash2, X } from 'lucide-react';
...
export default function Profile({person, badges, isOwner, works, openings, contributions, workFilter, view, stats, showcase, profileReadmeHtml, wallPosts}: {person: ProfilePerson; badges: Badge[]; isOwner: boolean; works: Pagination<Work>; openings: Opening[]; contributions: Work[]; workFilter: {kind: string; q: string}; view: string; stats: {followers: number; upvotes: number; collaborations: number}; showcase: Work[]; profileReadmeHtml: string; wallPosts: WallPost[]}) {
    ...
    return <section className="profile-page"><Head title={person.name}/><div className="profile-banner editable-block"><ProfileBanner person={person}/>{mediaEdit('banner', t.banner)}</div>
        ...
        {person.is_banned ? <p className="notice error">{t.profile_unavailable}</p> : <div className="profile-content">{(isOwner || showcase.length > 0 || profileReadmeHtml) && <section className="profile-showcase"><div className="section-heading"><h2>{t.showcase}</h2>{isOwner && <Link className="text-link" href="/profile/settings#showcase">{t.edit}</Link>}</div>{showcase.length > 0 && <div className="showcase-projects">{showcase.map(project => <Link key={project.id} href={project.url}>{project.cover && <img src={path(project.cover)} alt=""/>}<span><strong>{project.title}</strong>{project.subtitle && <small>{project.subtitle}</small>}</span></Link>)}</div>}{profileReadmeHtml && <Prose html={profileReadmeHtml}/>} {!showcase.length && !profileReadmeHtml && <p className="meta">{t.showcase_empty}</p>}</section>}
            ...
            {view === 'wall' && <section className="profile-section profile-wall"><p className="profile-wall-mode meta">{person.wall_mode === 'everyone' ? t.wall_everyone : t.wall_owner}</p>{canWriteWall && <WallComposer person={person} returnView="wall"/>}<div className="wall-list">{wallPosts.map(post => <article id={`wall-post-${post.id}`} key={post.id}><PersonLine person={post.author} detail={date(post.date)}/><p>{post.body}</p>{post.can_delete && <Form action={`/profile/wall/${post.id}`} method="delete"><button className="text-button danger" aria-label={t.delete}><Trash2 size={15}/>{t.delete}</button></Form>}</article>)}</div>{!wallPosts.length && <Empty>{t.wall_empty}</Empty>}</section>}
        </div>}
    </section>;
}
